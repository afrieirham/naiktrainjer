...
Everything in this document governs the whole site: `app/app.css` (the `@layer base` block and the `@theme` tokens `--color-paper`, `--color-band`, `--color-ink`, `--color-ink-soft`, `--color-rule`, `--color-rule-strong`, and `--font-sans: Archivo`), and every route — `app/routes/browse.tsx`, `app/routes/place.tsx`, `app/routes/contribute.tsx`, `app/routes/contributors.tsx` and the Access-gated `app/routes/admin.tsx`. The `--line-accent` token is set inline on the root of every public page from the selected Line in `data/network.ts`; the admin shell sets no Line and so falls back to ink. There is no scope to stay inside and no old world to leave alone; a new surface inherits these tokens and rules directly.
...
### Primary
- **Line Accent** (`#ed0f4c`, today the Kelana Jaya line's own colour): **read from `data/network.ts` (`lines[].color`) and set as `--line-accent` on the root of every public surface** — the Browse page, the Place page, the Contribute page and the Contributors page, each from the Line it shows. It is never hardcoded in the stylesheet. Used as the corridor spine over the stops with Places, the filled stop marker, the coverage diagram's filled run and markers, the `color-mix` tint on the selected row (8% at rest, 12% on hover), the `::selection` wash (24%), and the global focus ring. **Never used as text.**
...
### Line Selector
- **Placement:** the corridor header's own control, on the `flex-wrap` row that carries the 12px/700 uppercase Line name; it sits beside the name and before the Type filter, and wraps beneath them on a narrow screen.
- **Shape:** a native `<select id="line-selector" aria-label="Line">` — `paper` fill, 1px `rule-strong`, 6px radius, 12.5px/500 `ink`, `py-1 pl-2 pr-1.5`. Option text is `tabular-nums`. It is a control, so it takes **ink** as its selection language, never the accent.
- **Contents:** one option per **covered Line** — a Line that holds at least one Place — labelled with the Line's own name from `data/network.ts` and valued by its slug. A Line with no Places is absent, but the network still holds it, so a second covered Line is a data change and the control fills itself. Today the directory covers one Line and the control shows one option; it stays in place so the corridor reads as line-scoped rather than quietly hardcoded.
- **Behaviour:** the choice is written to `?line=<slug>`, and the default Line keeps a bare `/`. Switching is a client-side render that re-derives the corridor, its coverage, and `--line-accent` — the spine, markers, coverage diagram and selected-row tint all change with the Line — and clears any selected Place. An unknown, blank or stale slug resolves to the default Line, so a broken link still shows a corridor. Back, refresh and sharing restore the choice.
- **State:** the current Line is the selected value; every covered Line is one pick away. There is no disabled or loading state: the data is bundled, so the switch never waits.

### Station Picker (shared control)
The one control that chooses Stations, used by the Contribute form and both admin forms so the three cannot drift. It takes one or more Stations, and carries one optional Route frame per pick.

- **Shape:** a `fieldset` with a 12px/500 `ink-soft` legend (`Connections — the stations this place is near`), stacking one **Connection row** per Station. Each row is `grid gap-3 sm:grid-cols-[1fr_1fr_auto]`: the Station control, the optional Route frame field, and a `Remove` quiet button. A trailing `Add another station` quiet button sits below the rows. Rows are flat and held apart by the 8px gap and the legend, never by cards or an all-sides border.
- **Station control:** a native `<select>` styled exactly like the Type filter (`paper`, 1px `rule-strong`, 6px radius, 12.5px/500 `ink`). Options are grouped by Line with `<optgroup label={line.name}>`, and **every option reads `<code> <name>`** with the code in `tabular-nums` — `KJ27 Shang Villa`. An **Interchange** Station joins its twin codes into one option and names the other Lines it reaches: `AG7/SP7/KJ13 Masjid Jamek · Kelana Jaya, Sri Petaling`. One pick is one Connection, and an Interchange Connection places the Place on every twin Line with the same route.
- **Route frame:** one optional field per pick, placeholder `Route frame (optional)`. It accepts a whole `<iframe>` tag or a bare URL and is normalised to the embed URL; only a `google.com/maps/embed?pb=…` link passes, so a share link or a directions link is refused. Empty is valid and the Connection falls back to the Place's Map link.
- **States:** an empty picker is one blank row with its `Remove` disabled; each added row enables its own `Remove`; the last row cannot be removed. A row left blank is dropped on submit. An error renders once beneath the fieldset as a 12px/500 `ink` line — `Choose at least one station this place is near.`, `Every connection needs a station.`, `That Route frame is not a Google Maps embed link.` — never as per-row decoration.
...
- **Layout:** `min-h-dvh`, `mx-auto w-full max-w-[640px] px-5 py-10 sm:py-14`. A 26px → 34px display `h1` (`Contribute a place`), one `ink-soft` intro sentence, then the form at `mt-8` as a `flex flex-col gap-5`. The `640px` is a reading measure, not a card: no fill, no border, no shadow.
- **Controls, in order:** `Place name` (required, text, max 120); the shared **Station picker** (one or more Stations, each with an optional Route frame); `Type (optional)` (a select defaulting to `Not sure`); `Google Maps link` (required, `type="url"`, placeholder `https://`); `A note (optional)` (textarea, three rows, max 280); then `Your name (optional)` and `Your link (optional)` side by side at `sm`. Every field uses the shared frame — a 12px/700 uppercase 0.1em `ink-soft` label, the control, and a 12.5px/500 `ink` error beneath.
- **Copy:** the heading and the intro (`Know a place near a station? …`); the primary `Send for review`; the quiet aside beside it, `Reviewed by hand before it appears.`; field errors in the data's plain voice (`Give the place a name.`, `That link does not look like a web address.`); and a form-level error in a `band` panel with a `rule-strong` hairline and `role="alert"`.
- **States:** **rest** — the button enabled; **invalid** — per-field errors, no request made and no token spent; **sending** — `Sending…`, the button `disabled:opacity-60`, the Turnstile token fetched; **sent** — the form is replaced by a 26px → 34px `Thank you`, the pending-review sentence, and a `Browse places` primary action. The thank-you view is not a new page: the Contribute URL stays, so a refresh reaches the form again.
- **Accent:** the page sets `--line-accent` from the covered Line, so focus rings and the `::selection` wash carry the corridor's colour; the controls themselves stay ink.

### Contributors Page
The credit roll: who sent a Place in, and the Places they sent.

- **Layout:** the same `max-w-[640px] px-5 py-10 sm:py-14` column as the Contribute page, under the shared `AppBar` (subtitle the covered Line, action `Contribute a place`, no `Contributors` link on this page itself). A 26px → 34px display `h1` (`Contributors`) and one `ink-soft` sentence (`Every place here is either found by hand or sent in by someone. These are the people who sent one in.`).
- **List:** a `<ul>` opened by a `border-b border-rule`, each credit a `border-t border-rule py-4` row — flat and hairline-separated, never a card. The Contributor's name is 13.5px/600 `ink`, and a link when their own link exists, underlined `decoration-rule-strong underline-offset-4`, `target="_blank" rel="noopener noreferrer"`. Beneath it, 12.5px `ink-soft`, the Places they contributed, comma-separated, each a quiet link to its Place page. Rows run in the data's order — name, then link — never a hand-sorted order.
- **Empty state:** with nobody credited the list is absent and one `ink-soft` line stands in: `Nobody has contributed a place yet. Yours could be the first.`, the last sentence a quiet link to `/contribute/`.
- **Credit rule:** only a `contributed` Place credits anyone; an `owner` Place is credited to nobody even if a Contributor record lingers on it. A Contributor is their name *and* link, so the same name on two links is two people and the same link under two names is two people.

### Admin Shell & Forms
The maintainer's own surface, behind Cloudflare Access on `/admin` and `/api/admin/*` and marked `noindex, nofollow`. It is one prerendered shell at `/admin`; the mode lives in the query string (`?contribute=<id>`, `?place=<slug>`, `?place=new`), so no deep path or serving rewrite is needed. It wears the same tokens and controls as the public site and none of the public invitation.

- **Shell:** `min-h-dvh bg-paper`, the shared `AppBar` with subtitle `Admin` and action `Browse places`, and a `max-w-[720px] px-5 py-8` column — wider than the public measure because it is a working form, still flat and hairline-separated.
- **Admin index:** an `Admin` heading and one `ink-soft` sentence (`Approve a pending Contribution, or add and edit a Place by hand. Every change opens a pull request; merging publishes it on the next build.`). Two 12px/700 uppercase `ink-soft` section headings follow: `Approve a Contribution` — a `Contribution id` field plus an `Open` primary button — and `Places` — an `Add a Place` link over every Place as a 12.5px `ink-soft` link to `?place=<slug>`. It is a directory of the maintainer's work, not a dashboard: no counts, no cards.
- **Place and Approve forms:** the shared `Field` frame and the shared **Station picker**, exactly as the Contribute form uses them. Place adds `Kind` and `Type` side by side at `sm`, `Source` (`Researched by the maintainer` / `Contributed`), and a required `Map link`; Approve prefills every field from the Contribution and carries its Stations through untouched, needing no reshaping. The submit labels are `File the new place`, `File the edit`, and `Approve into a Place`. Every flow files a reviewable pull request; the write path's rules are the ones the form already ran.
- **States:** `Loading…` while a Contribution or an id resolves; `Could not load that contribution.` on failure; `No place lives at "…".` for an unknown slug; per-field errors from the same rules the endpoint runs; a `slug` conflict line (`A place already lives at "…". Edit it instead.`) under the form; a `band` panel for a form-level error; and a terminal view with no further controls — `Filed for review` with the pull-request link, or `Approved` naming the Place and the branch.
- **Accent:** the admin sets no Line, so `--line-accent` falls back to ink (`var(--line-accent, var(--color-ink))`, the Data-Owns-The-Colour Rule) and the focus ring is ink. The admin world is deliberately accent-free; it does not borrow a corridor it is not showing.
